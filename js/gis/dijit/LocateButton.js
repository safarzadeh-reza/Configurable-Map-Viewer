define([
    ...
    return declare(null, {
        growlTemplate: 'عرض جغرافیایی: {latitude}<br/>طول جغرافیایی: {longitude}<br/>دقت: {accuracy}<br/>ارتفاع: {altitude}<br/>دقت ارتفاع: {altitudeAccuracy}<br/>جهت: {heading}<br/>سرعت: {speed}',
        ...
        startup: function () {
            ...
            renderer.label = 'موقعیت جغرافیایی';
            renderer.description = 'موقعیت جغرافیایی';
            this.graphics.setRenderer(renderer);
            this.options.map.addLayer(this.graphics);
            this.options.graphicsLayer = this.graphics;
            this.options.infoTemplate = new InfoTemplate('موقعیت جغرافیایی', '${*}');
