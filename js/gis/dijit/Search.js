        ...
        return declare([_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin, _QueryBuilderMixin], {
            ...
            */
            ...
            addTopics: function () {
                ...
                this.own(topic.subscribe(this.topicID + '/returnGeometry', lang.hitch(this, 'returnGeometry')));
            ...
            *  Search Functions
            *******************************/
            returnGeometry: function (node, object) {
                var centerPoint;
                var graphic = this.geomforCalc;
                ...
                distParams.geometry2 = object.feature.geometry;
                distParams.geodesic = true;
                
                // get the field object id and change it to distance in order to handle printing and sorting options
                geometryService.distance(distParams, function (distance) {
                    result = Math.round(distance);
                    object.Dist=result
                    object.feature.attributes.Dist=result
                    node.innerHTML = '<i style=\'margin-left:2px; font-family: B Yekan; font-style: normal\'>' + result + '</i>';
                });
            }
            },
            ...
            getSearchTerm: function (input, field) {
                ...
                if (input.isValid && !input.isValid()) {
                    topic.publish('growler/growl', {
                        title: 'Search',
                        message: 'مقدار نامشخص برای ' + fieldName + '.',
                        level: 'error',
                        timeout: 3000
                    });
                    return null;
                ...
                if (searchTerm === '' && field.required) {
                    ...
                    topic.publish('growler/growl', {
                        title: 'Search',
                        message: ' مقداری برای جست و جوی ' + fieldName + 'انتخاب کنید.',
                        level: 'error',
                        timeout: 3000
                    });
                    return null;
                ...
                if (field.minChars && field.required) {
                    if (searchTerm.length < field.minChars) {
                        ...
                        topic.publish('growler/growl', {
                            title: 'Search',
                            message: 'مقدار جست و جو برای ' + fieldName + ' باید حداقل دارای ' + field.minChars + ' کارکتر باشد.',
                            level: 'error',
                            timeout: 3000
                        });
                        return null;
            ...
            *******************************/
            ...
            buildSearchControlLabel: function (field, search, layer, fieldNode) {
                ...
                if (field.minChars) {
                    title = 'حداقل ' + field.minChars + ' کارکتر وارد کنید';
                }
