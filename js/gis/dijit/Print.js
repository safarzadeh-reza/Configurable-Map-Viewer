...
define([
    ...
    var PrintDijit = declare([_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin], {
                ...
                if (layer.id === 'Measurement_graphicslayer') {
                    array.forEach(layer.featureCollection.layers, function (fcLayer) {
                        array.forEach(fcLayer.featureSet.features, function (feature) {
                            ...
                            feature.symbol.font.family = 'BYekan';
        ...
        _handlePrintInfo: function (data) {
            ...
            var choiceList = layoutTemplate[0].choiceList;//Added By Hakkak
            //var layoutItems = choiceList.filter(function (item){return item!="MAP_ONLY"}).map(function (item) {
            var layoutItems = choiceList.filter(function (item) { return item != "MAPS" }).map(function (item) {
                return {
                    name: item,
                    id: item
                };
