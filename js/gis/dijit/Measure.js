...
define([
    ...
    function (declare, _WidgetsInTemplateMixin, BaseWidget, _TemplatedMixin, GraphicsLayer, Edit, Graphic, Extent, Point,
        ...
        TextSymbol, Font, SpatialReference, esriUnits, scaleUtils, webMercatorUtils, geodesicUtils, geometryEngine, UndoManager, OperationBase,
        lang, on, html, Color, query, array, event, touch, has, dom, domConstruct, Select, NumberSpinner, ViewStack, SymbolChooser,
        DrawBox, jimuUtils, sniff, Widget, i18n) { /*jshint unused: false*/
        ...
        customOp.Add = declare(OperationBase, {
            ...
            constructor: function ( /*graphicsLayer, addedGraphics*/ params) {
                this.measureGraphicsLayer = params.graphicsLayer;
            ...
            performRedo: function () {
                array.forEach(this._addedGraphics, lang.hitch(this, function (g) {
                    this.measureGraphicsLayer.add(g);
                }));
            }
        });
        ...
        return declare([BaseWidget, _WidgetsInTemplateMixin], {
           // widgetsInTemplate: true,
            ...
            postCreate: function () {
                ...
                }).placeAt(this.measureSection, "after");;
                //html.place(this.viewStack.domNode, this.settingContent);

                this.textSymChooser.inputText.value = "متن نمونه";
                this.textSymChooser.textPreview.innerHTML = "متن نمونه";
                ...
                this.measureGraphicsLayer.id = this.id || this.name || 'emeasure';
                this.measureGraphicsLayer.name = "DrawAndMeasure";
                ...
                this.own(on(this.measureGraphicsLayer, "click", lang.hitch(this, function (evt) {
                    ...
                    html.setStyle(this.settingContent, 'display', 'none');
                    html.setStyle(this.textSection, 'display', 'none');
                    ...
                    if (evt.ctrlKey === true || evt.metaKey === true) {
                        ...
                        for (var i = this.measureGraphicsLayer.graphics.length - 1; i >= 0; i--) {
                            ...
                            if (g.attributes.id === id) {
                                deletedGraphics.push(g);
                                this.measureGraphicsLayer.remove(g);
                            }
                        }
                        this._pushDeleteOperation(deletedGraphics);
                        ...
                        if (evt.graphic.attributes.editable) {
                            if (this.editingEnabled === false) {
                                ...
                                html.setStyle(this.settingContent, 'display', 'block');
                                html.setStyle(this.textSection, 'display', 'block');
            ...
            _onDrawEnd: function (graphic, geotype, commontype) {
                ...
                // add all the relevant graphics associated with this graphic
                var addedGraphics = []
                var id = this.editGraphicID;
                for (var i = this.measureGraphicsLayer.graphics.length - 1; i >= 0; i--) {
                    var g = this.measureGraphicsLayer.graphics[i];
                    // put any measurement labels on top of drawn graphic
                    if (g !== graphic) {
                        this.measureGraphicsLayer.remove(g);
                        this.measureGraphicsLayer.add(g);
                    }
                ...
                this._pushAddOperation(addedGraphics);
                html.setStyle(this.settingContent, 'display', 'none');
                html.setStyle(this.textSection, 'display', 'none');
                html.setStyle(this.measureSection, 'display', 'none');
            },
            ...
            _editVertexStart: function (e) {
                ...
                if (isGhostVertex) {
                    ...
                    var a = {
                        id: this.editGraphic1.attributes.id,
                        index: this.editGraphic1.attributes.index + 1
                    };
            ...
            _measureDrag: function (e) {
                ...
                if (gra) {
                    ...
                    if (this.measureType === 'EXTENT') {
                        ...
                        var angle = this._calculateAngle(firstPoint, lastPoint);
                        var midPT = this._calculateMidPoint(firstPoint, lastPoint);
                        this._addSegmentMeasure(midPT, angle, 'طول : ' + segLength);
                        ...
                        this._addSegmentMeasure2(midPT, angle, 'عرض : ' + segLength);
                    }
                    if (this.measureType === 'CIRCLE') {
                        ...
                        if (has('ipad') || has('iphone') || has('android')) {
                            ...
                            var pt = new Point(e.mapPoint.x, e.mapPoint.y, this.map.spatialReference);
                            pt.y += this._calculateDistanceFromPixels(20);
                            this._addSegmentMeasure(pt, 0, 'شعاع :' + segLength);
                        } else {
                            this._addSegmentMeasure(e.mapPoint, 0, 'شعاع :' + segLength);
                        }
                    }
                    if (this.measureType === 'ELLIPSE') {
                        ...
                        var angle = this._calculateAngle(firstPoint, lastPoint);
                        var midPT = this._calculateMidPoint(firstPoint, lastPoint);
                        this._addSegmentMeasure(midPT, angle, 'a : ' + segLength);
                        var fPoint = Point(ext.xmin, ext.ymin);
                        var lPoint = Point(ext.xmin, ext.ymax);
                        var segLength = this._calculateSegmentLength(fPoint, lPoint);
                        var angle = this._calculateAngle(fPoint, lPoint);
                        var midPT = this._calculateMidPoint(fPoint, lPoint);
                        this._addSegmentMeasure2(midPT, angle, 'b : ' + segLength);
                    }
                    if (this.measureType == 'FREEHAND_POLYGON') {
            ...
            _calculateSegmentLength: function (pt1, pt2) {
                ...
                var precision = this._getDistanceUnitInfo(unit).precision || 1;
                var localeLength = jimuUtils.localizeNumber(geoLength.toFixed(precision));
            ...
            _addSegmentMeasure: function (pt, angle, length) {
                ...
                var symbolFont = new Font(this.textSymChooser.textFontSize.value + "px", a, b, c, "BYekan");
            ...
            _addLineMeasure: function (pl) {
                ...
                var localeLength = jimuUtils.localizeNumber(geoLength.toFixed(precision));
                var length = localeLength + abbr;
                var textSymbol = new TextSymbol("طول : "+length, symbolFont, fontColor);
                var att = {
                    id: this.editGraphicID,
                    index: 999
                };
            ...
            _addPolygonMeasure: function (pl) {
                ...
                var areaPrecision = this._getAreaUnitInfo(areaUnit).precision || 1;
                var geoArea = 0;
                if (this.useGeodesic) {
                ...
                var localeArea = jimuUtils.localizeNumber(geoArea.toFixed(areaPrecision));
                ...
                var lengthPrecision = this._getDistanceUnitInfo(lengthUnit).precision || 1;
                ...
                var localeLength = jimuUtils.localizeNumber(geoLength.toFixed(lengthPrecision));
                var length = localeLength + lengthAbbr;
                var textSymbol = new TextSymbol("مساحت : "+area, symbolFont, fontColor);
                ...
                var textLSymbol = new TextSymbol("محیط : "+length, symbolFont, fontColor);
            ...
            _getAllGraphics: function () {
                //return a new array
                return array.map(this.measureGraphicsLayer.graphics, lang.hitch(this, function (g) {
                    return g;
                }));
            },

            _onUndoManagerChanged: function () {
                html.setStyle(this.operationsSection, 'display', 'block');
            ...
            _pushAddOperation: function (options) {
                var graphics;
                if (options.graphic) {
                    var graphics = [options.graphic];
                } else if (lang.isArray(options)) {
                    graphics = options;
                }
                array.forEach(graphics, lang.hitch(this, function (g) {
                ...
                var addOperation = new customOp.Add({
                    graphicsLayer: this.measureGraphicsLayer,
            ...
            _pushDeleteOperation: function (options) {
                var graphics;
                if (options.graphic) {
                    var graphics = [options.graphic];
                } else if (lang.isArray(options)) {
